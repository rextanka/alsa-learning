...
| `CV_MIXER` | `inv_out` (out, PORT_CONTROL) | Inverted sum output — always −1 × `cv_out`; M-132 INV OUT precedent. Eliminates a downstream `INVERTER` when two destinations need opposing polarity from the same mix (e.g. filter opens while VCA closes). |
...
Our existing `CV_MIXER` covers the core M-132 summing capability. Phase 27E completes the picture by making the keyboard gate a first-class routable CV via `MIDI_CV.gate_cv`, and adding `CV_MIXER.inv_out` for the inverted sum output.
...
**Other M-132 use cases unlocked by `inv_out`:**

- **Counter-phase tremolo**: Route one LFO simultaneously to filter (opens) and VCA (closes) without an extra `INVERTER`. Connect `CV_MIXER.cv_out → VCF.cutoff_cv` and `CV_MIXER.inv_out → VCA.gain_cv`.
