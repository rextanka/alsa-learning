...
void osc_fill_buffer(Oscillator *osc, adsr_t *env, int16_t *buffer, int frames) {
    ...
    for (int i = 0; i < frames; i++) {
        // 1. Generate raw waveform (-1.0 to 1.0)
        double raw_val = osc_next_value(osc);

        // 2. Get current gain from ADSR state machine
        double gain = adsr_process(env);

        // 3. Apply gain and scale to S16_LE range
        int16_t sample = (int16_t)(raw_val * gain * amplitude);

        // 4. Stereo Interleaving
        buffer[i * 2]     = sample; 
        buffer[i * 2 + 1] = sample;
    }
}